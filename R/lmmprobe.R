...
lmmprobe <- function(
  ...
  R <- sapply(table(ID), diag, simplify = FALSE)
  invR <- lapply(R, solve)
  ...
  n_subj <- length(unique(ID)) # Precompute n_subj
  ...
  # sfLibrary(tidyr) - Not needed for future.apply
  ...
  ### Finishing up E-step ####
  if (sum(delta) == 0) {
    message("Delta is equal to zero")
    if (try2 == 0) {
      warning("Loop completely recycled back to beta=0.\n Trying again with different starting values. \n")
      count <- 0
      ...
      Xt_conv1 <- 1
      try2 <- 1
      W_ast <- rowSums(Z) * 0.0001 # apply(Z,1,sum) -> rowSums(Z)
      W_ast_var <- W_ast * 0
    } else {
      try2 <- 2
      conv_check <- conv_check + 1
      W_ast <- rowSums(Z) * 0.0001
      W_ast_var <- W_ast * 0
    }
  } else {
    #### Updating W, W2, b, b2 ####
  ...
  while (count < maxit & conv_check < 1) {
    ...
    if (all(W_ast == 0)) {
      message("all W are equal to zero")
      W_ast <- rowSums(Z) * 0.0001
      W_ast_var <- W_ast * 0
    }

    ### Precompute everything needed for calib and random effects ####
    #### Precompute inv_b_var once
    inv_b_var <- solve(b_var)
    ...
    b_vec_out_new <- b_vec_all[
      rep(seq_len(nrow(b_vec_all)), table(ID)), ,
      drop = FALSE
    ]
    ...
    if (sum(delta) == 0) {
      if (try2 == 0) {
        ...
        beta_adj_vec <- rep(0.0001, M)
        cor_vec <- rep(0, M)
        b_vec <- lapply(1:n_subj, matrix, data = rep(0, number_re), nrow = number_re, ncol = 1)
  ...
  VCV <- sigma2_c * XXt_c_inv %*% crossprod(Xt_c) %*% XXt_c_inv
  ...
  if (!is.null(Y_test) & !is.null(Z_test) & !is.null(V_test)) {
    ...
    by_j_calc_part2 <- future.apply::future_lapply(
    ...
    b_vec_new <- future.apply::future_lapply(
      1:length(unique(ID_test)),
      FUN = b_vec_fun,
      Tt_invR_T_test,
      sigma2_lmm,
    ...
    b2_vec_new <- future.apply::future_lapply(
      1:length(unique(ID_test)),
      FUN = b2_vec_fun,
      sigma2_lmm,
      Tt_invR_T_test,
    ...
    b_vec_out_new <- b_vec_all[
      rep(seq_len(nrow(b_vec_all)), table(ID_test)), ,
      drop = FALSE
    ]
    ...
    if (number_re == 1) {
      ...
      b_var_new <- matrix(
        NA,
        ncol = ncol(b2_vec_new[[1]]), nrow = ncol(b2_vec_new[[1]])
      )
