b_vec_func <- function(W_ast, ID, Tt_invR, Y_split, fit, intercept_split, X_split, Tt_invR_T, sigma2_lmm, inv_b_var, number_re) {
  ...
  by_j_calc_part2 <- future.apply::future_lapply(1:length(unique(ID)),
    FUN = by_j_calc_part2_fun, Tt_invR = Tt_invR,
    Y_split = Y_split, fit = fit, intercept_split = intercept_split,
    W_ast_split = W_ast_split, X_split = X_split
  )

  b_vec <- future.apply::future_lapply(1:length(unique(ID)), FUN = b_vec_fun, Tt_invR_T = Tt_invR_T, sigma2_lmm_value = sigma2_lmm, inv_b_var_value = inv_b_var, by_j_calc_part2 = by_j_calc_part2)
  if (number_re == 1) {
