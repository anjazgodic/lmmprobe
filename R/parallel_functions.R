...
b_vec_fun <- function(j, Tt_invR_T, sigma2_lmm_value, inv_b_var_value, by_j_calc_part2) {
  solve(Tt_invR_T[[j]] + sigma2_lmm_value * inv_b_var_value, by_j_calc_part2[[j]])
}

b2_vec_fun <- function(x, sigma2_lmm_value, Tt_invR_T, inv_b_var_value, b_vec) {
  solve((sigma2_lmm_value^(-1)) * Tt_invR_T[[x]] + inv_b_var_value) +
    tcrossprod(b_vec[[x]])
}
...
trace_eRe_fun <- function(x, Tt_invR_T, sigma2_lmm_value, inv_b_var_value, et_invR_e) {
  A <- (sigma2_lmm_value^(-1)) * Tt_invR_T[[x]] + inv_b_var_value
  sum(diag(solve(A, Tt_invR_T[[x]]))) + et_invR_e[x]
}

Vt_bb_by_j_part_fun <- function(x, sigma2_lmm_value, Tt_invR_T, inv_b_var_value) {
  solve((sigma2_lmm_value^(-1)) * Tt_invR_T[[x]] + inv_b_var_value)
}

Vt_Wb_by_j_part_fun <- function(x, Tt_invR_T, sigma2_lmm_value, inv_b_var_value, Tt_invR, W_ast_var_split) {
  solve(
    Tt_invR_T[[x]] + sigma2_lmm_value * inv_b_var_value,
    Tt_invR[[x]] %*% W_ast_var_split[[x]]
  )
}
