calib_final <- function(
  W_ast, ID, Y, W_ast_var, sigma2_lmm, Tt_invR_T, b_var, Tt_invR,
  curr_T_split, curr_T_split_t, b_vec, intercept, curr_T, b,
  number_re, X, Vt_bb_by_j_part = NULL, Vt_Wb_by_j_part = NULL
) {
  ##### Calibration model with current W and current b #####
  W_ast_split <- lapply(split(W_ast, ID), matrix, ncol = 1) # last iteration W from the E-Step

  if (is.null(Vt_bb_by_j_part) || is.null(Vt_Wb_by_j_part)) {
    W_ast_var_split <- lapply(split(W_ast_var, ID), matrix, ncol = 1) # last iteration W var from the E-Step
    inv_b_var <- solve(b_var)

    if (is.null(Vt_bb_by_j_part)) {
      Vt_bb_by_j_part <- future.apply::future_lapply(
        1:length(unique(ID)),
        FUN = Vt_bb_by_j_part_fun,
        sigma2_lmm_value = sigma2_lmm,
        Tt_invR_T = Tt_invR_T,
        inv_b_var_value = inv_b_var
      )
    }

    if (is.null(Vt_Wb_by_j_part)) {
      Vt_Wb_by_j_part <- future.apply::future_lapply(
        1:length(unique(ID)),
        FUN = Vt_Wb_by_j_part_fun,
        Tt_invR_T = Tt_invR_T,
        sigma2_lmm_value = sigma2_lmm,
        inv_b_var_value = inv_b_var,
        Tt_invR = Tt_invR,
        W_ast_var_split = W_ast_var_split
      )
    }
  }

  E_b1_squared <- unlist(future.apply::future_lapply(1:length(unique(ID)),
    FUN = E_b1_squared_fun,
    curr_T_split = curr_T_split, Vt_bb_by_j_part = Vt_bb_by_j_part,
    curr_T_split_t = curr_T_split_t, b_vec = b_vec
  )) # Var b1 + Eb1*Eb1

  E_Wb1_c <- unlist(future.apply::future_lapply(1:length(unique(ID)),
    FUN = E_Wb1_c_fun,
    curr_T_split = curr_T_split, Vt_Wb_by_j_part = Vt_Wb_by_j_part,
    W_ast_split = W_ast_split, b_vec = b_vec
  )) # Cov W-b1 + EW*Eb1

  cov_Wb1_c <- unlist(future.apply::future_lapply(1:length(unique(ID)),
    FUN = cov_Wb1_c_fun,
    curr_T_split = curr_T_split, Vt_Wb_by_j_part = Vt_Wb_by_j_part
  )) # Cov W-b1

  if (number_re == 1) {
    E_b2_squared <- E_b1b2_c <- E_Wb2_c <- cov_b1b2_c <- cov_Wb2_c <- NULL
  } else {
    E_b2_squared <- unlist(future.apply::future_lapply(1:length(unique(ID)),
      FUN = E_b2_squared_fun,
      curr_T_split = curr_T_split, Vt_bb_by_j_part = Vt_bb_by_j_part,
      curr_T_split_t = curr_T_split_t, b_vec = b_vec
    )) # Var b2 + Eb2*Eb2

    E_b1b2_c <- unlist(future.apply::future_lapply(1:length(unique(ID)),
      FUN = E_b1b2_c_fun,
      curr_T_split = curr_T_split, Vt_bb_by_j_part = Vt_bb_by_j_part,
      curr_T_split_t = curr_T_split_t, b_vec = b_vec
    )) # Cov b1-b2 + Eb1*Eb2

    E_Wb2_c <- unlist(future.apply::future_lapply(1:length(unique(ID)),
      FUN = E_Wb2_c_fun,
      curr_T_split = curr_T_split, Vt_Wb_by_j_part = Vt_Wb_by_j_part,
      W_ast_split = W_ast_split, b_vec = b_vec
    )) # Cov W-b2 + EW*Eb2

    cov_b1b2_c <- unlist(future.apply::future_lapply(1:length(unique(ID)),
      FUN = cov_b1b2_c_fun,
      curr_T_split = curr_T_split, Vt_bb_by_j_part = Vt_bb_by_j_part,
      curr_T_split_t = curr_T_split_t
    )) # Cov b1-b2

    cov_Wb2_c <- unlist(future.apply::future_lapply(1:length(unique(ID)),
      FUN = cov_Wb2_c_fun,
      curr_T_split = curr_T_split, Vt_Wb_by_j_part = Vt_Wb_by_j_part
    )) # Cov W-b2
  }

  if (number_re == 1) {
    Xt_c <- as.matrix(cbind(intercept, W_ast, curr_T[, 1] * b[, 1]))
  } else {
    Xt_c <- as.matrix(cbind(intercept, W_ast, curr_T[, 1] * b[, 1], curr_T[, 2] * b[, 2], X[, "x_adj"]))
  }

  XXt_c <- crossprod(Xt_c)
  XXt_c[2, 2] <- sum(W_ast_var + (W_ast^2))
  XXt_c[2, 3] <- sum(E_Wb1_c)
  XXt_c[3, 3] <- sum(E_b1_squared)
  XXt_c[3, 2] <- sum(E_Wb1_c)

  if (number_re == 2) {
    XXt_c[4, 4] <- sum(E_b2_squared)
    XXt_c[2, 4] <- sum(E_Wb2_c)
    XXt_c[4, 2] <- sum(E_Wb2_c)
    XXt_c[3, 4] <- sum(E_b1b2_c)
    XXt_c[4, 3] <- sum(E_b1b2_c)
  }

  XXt_c_inv <- solve(XXt_c)
  c_coefs <- solve(XXt_c, crossprod(Xt_c, Y))

  out <- list(
    c_coefs = c_coefs, Xt_c = Xt_c, XXt_c_inv = XXt_c_inv,
    Vt_bb_by_j_part = Vt_bb_by_j_part, cov_Wb1_c = cov_Wb1_c,
    cov_b1b2_c = cov_b1b2_c, cov_Wb2_c = cov_Wb2_c
  )

  return(out)
}
