calib_final <- function(W_ast, ID, W_ast_var, sigma2_lmm, Tt_invR_T, b_var, Tt_invR, 
  ...
  cov_Wb1_c <- unlist(sfLapply(1:length(unique(ID)), fun=cov_Wb1_c_fun, 
                               curr_T_split=curr_T_split, Vt_Wb_by_j_part=Vt_Wb_by_j_part)) #Cov W-b1 
  ...
  if(number_re == 1){
    ...
    cov_b1b2_c <- unlist(sfLapply(1:length(unique(ID)), fun=cov_b1b2_c_fun, 
                                  curr_T_split=curr_T_split, Vt_bb_by_j_part=Vt_bb_by_j_part, 
                                  curr_T_split_t=curr_T_split_t)) #Cov b1-b2 
    
    cov_Wb2_c <- unlist(sfLapply(1:length(unique(ID)), fun=cov_Wb2_c_fun, 
                                 curr_T_split=curr_T_split, Vt_Wb_by_j_part=Vt_Wb_by_j_part)) #Cov W-b2 
  }
