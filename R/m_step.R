m_step_it2plus <- function(M, N, W_ast_var, ID, number_re, curr_T, b, sigma2_lmm, Tt_invR_T,
  ...
  Vt_bb_by_j_part <- future.apply::future_lapply(1:n_subj,
  ...
  Vt2_2 <- unlist(lapply(1:n_subj, function(x) Vt2_2_fun(x, curr_T_split, Vt_bb_by_j_part, curr_T_split_t))) # Var b1
  Vt1_2 <- Vt2_1 <- unlist(lapply(1:n_subj, function(x) Vt1_2_fun(x, curr_T_split, Vt_Wb_by_j_part))) # Cov Wb1, Cov b1W

  if (number_re == 1) {
    Vt2_3 <- Vt3_2 <- Vt3_3 <- Vt1_3 <- NULL
    Vt <- cbind(c(Vt1_1, Vt2_1), c(Vt1_2, Vt2_2), c(Vt1_3, Vt2_3))
  } else {
    Vt1_3 <- Vt3_1 <- unlist(lapply(1:n_subj, function(x) Vt1_3_fun(x, curr_T_split, Vt_Wb_by_j_part))) # Cov Wb2, Cov b2W
    Vt2_3 <- unlist(lapply(1:n_subj, function(x) Vt2_3_fun(x, curr_T_split, Vt_bb_by_j_part, curr_T_split_t))) # Cov b1b2
    Vt3_2 <- unlist(lapply(1:n_subj, function(x) Vt3_2_fun(x, curr_T_split, Vt_bb_by_j_part, curr_T_split_t))) # Cov b2b1
    Vt3_3 <- unlist(lapply(1:n_subj, function(x) Vt3_3_fun(x, curr_T_split, Vt_bb_by_j_part, curr_T_split_t))) # Var b2
  ...
  if (number_re == 1) {
    LR_cpp <- UNHIDEM_cpp0_5(
      Y, as.matrix(Z), as.matrix(Vt),
      rowind1, rowind2,
      colind1, colind2,
  ...
  out <- list(
    mu_m = mu_m, beta_m = beta_m, alpha_m = alpha_m, gamma_m = gamma_m,
