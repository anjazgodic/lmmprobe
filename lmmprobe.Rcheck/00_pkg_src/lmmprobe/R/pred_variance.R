pred_var <- function(W_ast, ID, W_ast_var, VCV,
  ...
  if (number_re == 1) {
    ...
    V_b <- unlist(lapply(seq_along(unique(ID)), function(x) Vt2_2_fun(x, curr_T_split, Vt_bb_by_j_part, curr_T_split_t))) # Var b1
    V_wb <- cov_Wb1_c
    S2_mu <- VCV[1, 1]
    S2_alpha <- VCV[2, 2]
    S2_gamma <- VCV[3, 3]
    ...
    V_w <- W_ast_var + (W_ast^2)
    V_b1 <- unlist(lapply(seq_along(unique(ID)), function(x) Vt2_2_fun(x, curr_T_split, Vt_bb_by_j_part, curr_T_split_t))) # Var b1
    V_b2 <- unlist(lapply(seq_along(unique(ID)), function(x) Vt3_3_fun(x, curr_T_split, Vt_bb_by_j_part, curr_T_split_t))) # Var b2
    V_b1b2 <- cov_b1b2_c
    V_wb1 <- cov_Wb1_c
    V_wb2 <- cov_Wb2_c
    S2_mu <- VCV[1, 1]
